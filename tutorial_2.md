...
```
    BASE.sh --analyze -i complete_OGs/ -o complete_OGs_analyze 
...
```
    sh BASE.sh --extract -i complete_OGs_annotate -l branch.lst -n 1
```
...
each line of the label file generates an output, named with the name of their clade /tip and the treshold of missing data (min.otu, where otu stands for Operational Taxonomic Units). This is an ouput for a clade:
...
Typing ```column -t complete_OGs_annotate/branch.single_species.min.otu.1.dNdS.summary``` you can see how an output for a terminal branche should look like:
