...
* a [folder](https://github.com/for-giobbe/BASE/tree/master/example/_ubiquitous_OGs) containing aligned OGs ```.fa```. These alignment shouldn't have any STOP codon, 
yet BASE will report the OGs wehre they are found. The fasta alignment headers have to be identical to the tips names in the phylogenetic tree.
...
When using your own data, remember to properly set the genetic code in the ```.ctl``` files. Moreover remember that species name should match exactly between OGs and the phylogeny.

---

We can start the analysis using 4 cores (```-- cores``` 4) and restricting the analysis to ubiquitous OGs, using the line:
...
This is because this OG misses one of the 11 species which are present in the species tree; we'll see how to included it furhter in the tutorial.
...
A rather important file is the summary of the LRTs. Type ```column -t column -t _ubiquitous_OGs_0VS1/likelihood_summary.txt``` to take a look at it:
...
```
2*(general model lnL - alternative model lnL)
...
Subsequently we can proceed to the further step, ```--extract```, whcih
takes as input the folder generated by the previous step - which containt the codeml outputs - and will generate
an equivalent number of  ```.result ``` files, which contain:

* the internal nodes annotation created by codeml
* the species which are associated to each phylogeny branch. 
...
form the clade relative to our branch of interested - separated by single spaces - followed by an identifier. Here clade_of_interest and species_of_interest are used as identifiers
for a clade and a species, but of course they can be changed to actual clades/species names.
If we are interested in a terminal branch relative to a single species, just specify the species name in the tree and an identifier.
...
```
    sh ../BASE.sh --extract --input _ubiquitous_OGs_0VS1 --labels branch.lst --min_spp 1
```
...
each line of the label file generates an output, named with the name of their clade /species and the treshold of missing data 
