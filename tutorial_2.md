...
Moreover remember that species name should match exactly between OGs and the phylogeny.
...
an equivalent number of  ```.result ``` files, which contain for each OG:

* trees with internal nodes matching codeml outputs
...
As we launched the extract without specifying any branch/clade this step just formatted the outputs for the annotation;
to retrive the metric relative to our branch/clade of interest we need to specify them using a file like [this](https://github.com/for-giobbe/BASE/blob/master/example/branch.lst) 
...
Here branch_of_interest and species_of_interest are used but of course they can be changed to any name. Let's take a look to our labels file:
...
as the workflow will recognize the ```.result``` files and skip that step.
...
each line of the label file generates asummary output, named with the identifyer and the treshold of missing species 
