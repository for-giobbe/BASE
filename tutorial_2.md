...
When using your own data, remember to properly set the genetic code in the ```.ctl``` files and 
remember that species name should match exactly between OGs and phylogeny.

---

We can start the analysis using 4 cores - using the ```--cores``` flag - and restricting the analysis to ubiquitous OGs - using the ```--ubiquitous``` flag:
...
rather important output is already the summary of the LRTs itself. Type ```column -t _ubiquitous_OGs_0VS1/likelihood_summary.txt``` to take a look at it:
